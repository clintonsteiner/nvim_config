...
wk.register({
    ...
    c = {name = "change dir",
        ...
        -- d = {"<cmd>lua require('fzf-lua').fzf_exec([[(echo '..' ; echo '-' ; echo '~' ; fd --type=d --follow --hidden --exclude=.git 2>/dev/null)]], {prompt = 'Cd> ', previewer = false, actions = {['default'] = function(selected) vim.api.nvim_command('cd ' .. selected[1]) end}})<CR>", "change dir"},
    },
    g = {name = "git",
    ...
    l = {name = "lsp",
        ...
        f = {"<cmd>lua darker()<CR>", "apply darker"},
    ...
    o = {name = "open",
        f = {"<cmd>lua require('fzf-lua').files({cwd_prompt = false, prompt = 'Files> ', git_icons = false})<CR>", "files"},
    ...
    r = {name = "ripgrep",
        g = {"<cmd>lua require('fzf-lua').live_grep({git_icons = false})<CR>", "rg"},
        l = {"<cmd>lua require('fzf-lua').grep_last()<CR>", "rg last"},
        r = {"<cmd>lua require('fzf-lua').grep_cword({multiline = 2})<CR>", "rg word under cursor"},
    },
    t = {name = "toggles",
